...
public:
  ...
  bool isLinked(const Edge *otherEdge) const {
    if (otherEdge) {
      ...
      while (e) {
        if (e->getData() == otherEdge ||
            e->getData()->getAnotherEnd((*otherEdge)[0]) ||
            e->getData()->getAnotherEnd((*otherEdge)[1]))
          return true;
        e = e->getNext();
      }
    }
  ...
  do {
    ...
    while (curVertex) {
      ...
      if (!visitedVertices[curVertex->getData()]) {
        std::cout << "Vertex " << curVertex << " not fully visited yet"
                  << std::endl;
        ...
        std::cout << "Current Adjacent list(" << curAdjList.size() << ")";
        ...
        for (const auto &adjVertex : curAdjList) {
          ...
          if (minSpanTree.isLinked(tmpEdge)) {
    ...
    if (minEdge && !visitedEdges[minEdge]) {
      std::cout << "Checking min edge: " << minEdge << std::endl;
      ...
      std::cout << "minSpanTree: ";
      tmpEdgeNode = minSpanTree.getEdgeList()->getHead();
      while (tmpEdgeNode) {
        std::cout << tmpEdgeNode << ", ";
        tmpEdgeNode = tmpEdgeNode->getNext();
      }
      std::cout << std::endl;
      ...
      std::cout << "Min edge found: " << minEdge << std::endl;
    }
    print("End of loop");
  } while (minEdge);
