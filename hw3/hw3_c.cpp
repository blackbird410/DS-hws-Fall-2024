...
template <class T> class Tree {
  ...
  bool isTreeReversed() {
    ...
    return (root->LeftChild && root->LeftChild > root || root->RightChild && root->RightChild < root);
  };

  T* Get(TreeNode<T>* node, T target) const {
    if (!node) return nullptr;
    if (target == node->data) return &node->data;

    if (isTreeReversed())
      return (node->data < target) ? Get(node->RightChild, target) : Get(node->LeftChild, target);
    else 
      return (node->data > target) ? Get(node->RightChild, target) : Get(node->LeftChild, target);

  };

  bool deleteNode(TreeNode<T>* node, T value) {
...
template <class T> void Tree<T>::reverse() { reverse(root); };
template <class T> T* Tree<T>::Get(T value) const { return Get(root, value); };
template <class T> bool Tree<T>::delete(T value) { return deleteNode(root, value); };
